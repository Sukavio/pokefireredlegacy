...
static void Cmd_getexp(void)
{
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            || levelCappedNuzlocke(GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL, NULL)))
            {
                *(&gBattleStruct->sentInPokes) >>= 1;
                gBattleScripting.getexpState = 5;
                gBattleMoveDamage = 0; // used for exp
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_CHECK_POKEFLUTE:
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            abilityNum = GetMonData(&gPlayerParty[i], MON_DATA_ABILITY_NUM, NULL);
            status = GetMonData(&gPlayerParty[i], MON_DATA_STATUS, NULL);
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            species = GetMonData(&gEnemyParty[i], MON_DATA_SPECIES_OR_EGG, NULL);
            abilityNum = GetMonData(&gEnemyParty[i], MON_DATA_ABILITY_NUM, NULL);
            status = GetMonData(&gEnemyParty[i], MON_DATA_STATUS, NULL);
...
#define MON_CAN_BATTLE(mon) (((GetMonData(mon, MON_DATA_SPECIES, NULL) && GetMonData(mon, MON_DATA_IS_EGG, NULL) != TRUE && GetMonData(mon, MON_DATA_HP, NULL))))
    ...
    else
    {
        ...
        for (;gBattleCommunication[0] < PARTY_SIZE; gBattleCommunication[0]++)
        {
            if (GetMonData(&party[gBattleCommunication[0]], MON_DATA_HP, NULL)
                && GetMonData(&party[gBattleCommunication[0]], MON_DATA_SPECIES_OR_EGG, NULL)
...
static void Cmd_pickup(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (GetMonData(&gPlayerParty[i], MON_DATA_ABILITY_NUM, NULL) != ABILITY_NONE)
