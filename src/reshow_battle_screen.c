...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        else if (GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], MON_DATA_HP, NULL) == 0)
        {
            return;
        }
        else
...
static void CreateHealthboxSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        else if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI) && GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], MON_DATA_HP, NULL) == 0)
        {    
            SetHealthboxSpriteInvisible(healthboxSpriteId);
        }
    }
